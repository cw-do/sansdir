...
access to that path, so no per-user installation step is required. The program
is launched by running that script from any terminal on the cluster---a login
node, a web-terminal session, or an \ac{ssh} connection:
