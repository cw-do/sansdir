...
\input{sansdir-abbreviations.tex}
...
use for \ac{sns} \ac{sans} workflows and is installed cluster-wide on the
analysis cluster, launched simply as \texttt{sansdir}.
...
rewrite. The implementation is approximately 18,400 lines of Python across 58
modules, covered by 799 automated tests, and starts in well under the
300~ms budget.
...
Section~\ref{sec:oncat}; that client was written fresh for this program rather
than by copying \texttt{eqsanscli}'s source code into it.

\subsection{SCOPE OF THIS REPORT}

This report describes version 0.10.2. Section~\ref{sec:install} covers
...
program is installed cluster-wide as the command \texttt{sansdir}: a released,
non-editable install with its own self-contained Python environment, so no
per-user installation step is required. It is launched from any terminal on
the cluster---a login node, a web-terminal session, or an \ac{ssh}
connection:
...
The first form opens the interface in the current working directory. The optional
argument is a starting path: the directory the left pane opens into, so a
user can jump straight to a proposal's \texttt{shared} folder without navigating
there by hand. The right pane mirrors it initially. Any readable directory is
accepted; a path under \texttt{/SNS/USANS} additionally starts the program in
\ac{usans} mode (Section~\ref{sec:usans}).

Should \texttt{sansdir} not be on \texttt{PATH} on a particular machine, the
same launcher is always reachable as
\texttt{/SNS/EQSANS/shared/bin/sansdir}.

\subsection{PORTABILITY OF THE LAUNCHER}

The entry-point script may also simply be copied or symlinked anywhere on the
filesystem and executed from there. This works because the script's shebang
line names an \emph{absolute} interpreter path inside the shared virtual
environment, so the kernel executes the bundled Python regardless of where
the script file itself resides. The resulting execution chain is:

\begin{lstlisting}
[user's copy of the script]
  -> <install>/.venv/bin/python        (via absolute shebang)
     -> the installed sansdir package  (inside that venv)
\end{lstlisting}
...
\begin{figure}[ht]
  \centering
  \resizebox{\textwidth}{!}{\input{figures/fig-architecture.tex}}
...
Python allows defining a one-line helper function without giving it a name (a
\emph{lambda}), but a nameless function identifies itself in error messages
only as \texttt{<lambda>}. Every resolver here is therefore an ordinary
function with a descriptive name, so an error traceback and the help overlay
can both say which resolver was involved. The default map contains 48
...
The design budget is 300~ms from invocation to first paint. Re-measured after
the interface grew the \ac{usans} reduction and desmearing features, the cold
start for \texttt{sansdir --help} remains well inside that budget: the median
end-to-end wall-clock time on an analysis node is about 120~ms, of which roughly
40~ms is Python interpreter start-up and about 40~ms is importing the command
module; the best case is near 60~ms. The figure did not grow with the new
features because the fast path imports none of them---a check confirms that
\texttt{numpy}, \texttt{h5py}, \texttt{matplotlib}, Textual, and \texttt{httpx}
are all absent from \texttt{sys.modules} after \texttt{import sansdir.cli}.
...
approximately 18,400 lines across 58 modules.
...
\begin{table}[ht]
  ...
  \begin{tabular}{l p{0.68\textwidth}}
    ...
    \texttt{commands/} & \texttt{registry.py} (the \texttt{Command} model and
      dispatch), \texttt{builtins.py} (all 56 registrations),
...
the filename as each subplot title. Two color-bar modes are available. In
\emph{shared} mode a single scale spans all panels, so intensities are directly
comparable; in \emph{independent} mode each panel is scaled to itself, so weak
patterns remain visible. Shared is the default, on the reasoning that
side-by-side panels are usually being compared. The choice is made through the
\texttt{[ui].colorbar\_mode} configuration key, which the \key{p} key reads as a
standing preference; a single plot can override it from the command line with
\texttt{:plot.iqxqy colorbar\_mode=independent}, whose argument tab-completes to
...
Figure~\ref{fig:metadata-workflow} walks through a complete extract-and-plot
session on a real experiment file, exactly as a user at the terminal would see
it. The file is a Mantid-processed workspace from an EQ-SANS measurement with an
in-situ mechanical load frame, and the quantity of interest is the load-cell
record from that sample environment---a \ac{das} log named
\texttt{BL6:SE:PsyloTech:SGLC}---which the user wants as a plottable time
series.

\begin{enumerate}
  \item Navigate the active pane to the file
  ...
  \item Press \key{M}. The key picker opens on the file's HDF5 tree. Nobody
    remembers a full \ac{das} log path, so press \key{/} and type a fragment
    of the name: \texttt{LC}. Three rows match; \key{Space} on
    \texttt{.../SGLC/time} and \texttt{.../SGLC/value} stars both, and the
    status line confirms \emph{selected: 2 key(s)}
    (panel~a).
  \item Press \key{Ctrl+S}. The picker folds back into the output form with
    both keys filled in. Per-file mode and the
    \texttt{<filename>\_extracted} template are already the defaults; switch
    the format to \ac{csv} and press \key{Ctrl+S} again to run
    (panel~b).
  \item The extractor writes
    \texttt{<filename>\_extracted.csv}---two columns, \texttt{time} and
    \texttt{value}, 5\,644 rows---into the inactive pane's directory, and
    that pane refreshes with the cursor ready to move onto the new file
    (panel~c).
  \item Press \key{Tab} to switch panes, put the cursor on the new \ac{csv},
    ...
    axis labels and draws value against time (panel~d): the load ramps
    during the first 450~seconds of the run and releases sharply---precisely
    the kind of sanity check that would otherwise require exporting the file
    and opening a separate analysis tool.
\end{enumerate}

The four panels were captured from the running program, so fonts, colors, and
layout are what the terminal actually shows.

\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{figures/metadata-workflow.pdf}
  \longcaption{Batch metadata extraction, end to end.}{(a)~The key picker
  ...
  chosen keys, per-file mode, and CSV format. (c)~The destination pane after
  the run: the extracted CSV is under the cursor. (d)~The curve produced by
  pressing \key{l} on it---the load-cell trace over the course of the run.}
...
\Ac{usans} overlaps almost entirely with what the program already does: the same
proposal catalog, the same file manager, the same one-dimensional plotting, the
same editor. What it adds is a short reduction workflow and, downstream of it, a
desmearing step. The reduction itself is delegated to the instrument team's
\texttt{usansred} package; the desmearing implements the truncated Abel inversion
of Huang \autocite{huang2026abel}. Rather than ship a second application, the
program carries an \emph{instrument mode} and a small, self-contained module.

\subsection{INSTRUMENT MODE}

The mode is either \ac{sans} or \ac{usans}, and it is resolved once at launch:
path detection first, then \texttt{[instrument].default}, then
\texttt{[oncat].default\_instrument}. Any launch path containing
\texttt{usans}---which covers \texttt{/SNS/USANS/IPTS-\textit{N}} as well as a
user's own scratch directories---selects \ac{usans}. Path detection outranks the
configuration file because a user who opens a \ac{usans} proposal directory means
it, whatever their defaults say. The command \texttt{instrument.set}, reached as
\texttt{:instrument usans}, switches at runtime.

The mode governs four things: which instrument the proposal browser queries,
which columns the run catalog shows (\ac{usans} has no moveable detector and a
fixed incident wavelength, so the distance and wavelength columns would be
constants and are dropped), what \key{Enter} does on a catalog row, and whether
the \key{r} binding is live. It is signalled in the title bar and by the presence
of \texttt{r:Reduce} in the key hint bar.

Mode scoping is applied to \emph{bindings}, not to registrations. The
\ac{usans} commands are registered in both modes, so \texttt{:usans-reduce}
works whether or not the user remembered to switch; only the key disappears.
Correspondingly, \texttt{default\_keymap()} called with no argument returns
exactly the map it returned before \ac{usans} existed, which is what the help
overlay, the hint bar, and the existing test suite all observe.

\subsection{THE SETUP TABLE}

The reduction engine is driven by a small \ac{csv} listing, one row per sample:
a flag distinguishing the empty-cell background from the samples, a name that
becomes the output filename stem, a first run number, a scan count, a thickness,
and optionally runs to exclude. The program generates this table from the
proposal's run list and then gets out of the way: the user reviews and corrects
it with \key{F4}, the same editor used for any other file. There is deliberately
no table-editing widget.

Generation groups consecutive runs sharing a title into blocks and picks the
block whose title names an empty cell or banjo as the background. The
load-bearing step is the scan count. A titled block of $N$ consecutive runs is
$(N-1)$ analyzer rocking-curve scans plus one transmission run, and the engine
reads only the rocking-curve files. $N$ is not a constant: it varies with the
experiment's settings. Passing the run count that the catalog reports therefore
makes the engine fail on the transmission run's absent input. The program instead
derives the count from which runs actually have rocking-curve files on disk,
which handles any block size without configuration, and expresses any gap inside
a block through the \ac{csv}'s exclude column.

Some judgements cannot be made from the data alone. A measurement that was
stopped and restarted under one title leaves an oversized block in which only
the last few runs are real; the generator narrows to the last window of the modal
size and flags the block. Every such judgement---narrowed blocks, unusual sizes,
blocks with no reducible runs, the transmission runs that were dropped, the
background that was chosen---is written to a companion notes file. The generated
table is a first draft, and the notes exist so that reviewing it is quick rather
than archaeological.

\subsection{INVOKING THE ENGINE}

The reduction mathematics belongs to the instrument team's \texttt{usansred}
package, deployed on the cluster as a pixi environment. The program locates its
console script and runs it as a subprocess; it does not reimplement the
physics, and no code from \texttt{usansred} is copied into this code base.
Two deployment realities are absorbed on the caller's side.

First, the console script inherits the invoking user's \texttt{\textasciitilde
/.local} site packages and aborts on a stale \texttt{pandas}, which is reachable
on the analysis nodes. Every invocation therefore disables user site packages and
empties \texttt{PYTHONPATH}.

Second, the engine resolves its per-run input relative to the directory holding
the setup \ac{csv}, not the working directory. The reviewed table normally lives
in the user's own folder, and the pre-processed data lives under the proposal's
autoreduce directory---which is instrument data the program must not write to,
and which is frequently read-only to the user in any case. Rather than copy the
table into it, the program builds a temporary directory of symbolic links to the
data, places a copy of the table there, and runs the engine in that directory.
Nothing beneath the instrument data root is modified.

\subsection{ONE KEY}

The two operations are exposed through a single binding. \key{r} reduces the
setup table under the cursor; when the cursor is not on one, it offers to build
a table for the proposal named by the current path, fetching the run list,
showing the catalog beside the user, and leaving the cursor on the file it wrote.

It stops there rather than reducing immediately. The table is a first draft whose
guesses the notes exist to expose, so the second \key{r} is the review step, and
it costs one keystroke. The one case that does \emph{not} offer to regenerate is
a table that parses but fails validation---two backgrounds, a zero scan count:
there the user is mid-edit on a real table, and regenerating would discard the
corrections. That case reports the problem and points at \key{F4}.

Reduced output is ordinary two- or three-column text, so the existing plotting
path handles it with only a filename rule added to the classifier. The rule
matches on the name rather than the column count so that a sample whose name
happens to contain \texttt{trans} is not routed to the transmission plotter.

The engine's final curve carries a verbose suffix,
\texttt{\_background\_subtracted.txt}. Because that name is long, the program
writes a shorter \texttt{\_bsub.txt} alias beside it after each reduce, keeping
the original so the engine's own summary spreadsheet and any downstream tooling
that expects the standard name continue to work. The alias is a copy, not a
symbolic link, and is controlled by \texttt{[usans].short\_name\_copy}.

\subsection{A WORKED EXAMPLE}
\label{sec:usans-example}

Figure~\ref{fig:usans-workflow} shows the complete flow on a real proposal
(IPTS-37679), from an empty working folder to reduced curves, using only the
\key{r} key and the prompts it raises.

\begin{enumerate}
  \item Open the program in a writable working folder anywhere under the
    proposal's path. The \texttt{/SNS/USANS} prefix puts it in \ac{usans} mode
    automatically.
  ...
  \item The run list is fetched from the catalog and displayed in the other
    ...
    49434 (panel~b). Blocks recorded before that run---beam-cycle alignment
    and test scans---are noted but left out of the table.
  \item The program writes \texttt{IPTS-37679\_setup.csv} and its companion
    ...
    in the other pane (panel~c): here one background and 23 sample blocks.
    This is the review gate---check the background pick and block sizes
    against the NOTE, editing with \key{F4} if needed.
  \item Press \key{r} again, now on the table. The data directory defaults to
    ...
    every block---seconds, since it works from the pre-processed per-run
    ASCII. On completion the program offers to show the output folder in
...
\begin{figure}[ht]
  \centering
  \includegraphics[width=\textwidth]{figures/usans-workflow.pdf}
  \longcaption{USANS reduction, end to end, on one key.}{(a)~\key{r} in an
  empty working folder offers to build the setup table for the IPTS named by
  the path. (b)~The start-of-experiment prompt (run 49434); the fetched run
  list is visible in the other pane. (c)~The generated table under the
  cursor, previewed in the other pane for review. (d)~After the second
  \key{r}: the reduced curves in the output folder.}
...
\Ac{usans} instruments use slit rather than pinhole collimation: the larger
illuminated area buys the flux that the very high angular resolution of the
Bonse--Hart crystal optics demands. The cost is that the recorded intensity at a
nominal momentum transfer $Q$ is an average of the true intensity over a tall
vertical acceptance,
%
\begin{equation}
...
instrument $\sigma_y \simeq 0.13~\text{\AA}^{-1}$, two decades above the top of
the measured window. Recovering $I(Q)$ from $I_\text{exp}(Q)$ is the desmearing
problem, and it is ill-posed: the slit average couples each measured point to
scattering at much higher $Q$.

The \key{d} key desmears the selected reduced curves. The implementation is the
non-iterative truncated Abel inversion of Huang \autocite{huang2026abel}, which
expresses the intrinsic intensity in closed form as
%
\begin{equation}
...
\end{equation}
%
Being closed-form, it propagates uncertainty through a single linear operator
rather than accumulating it over iterations, which is the principal weakness of
the widely used Lake algorithm \autocite{lake1967} it replaces. Every integral
is evaluated with the substitution $Q_x = \sqrt{Q^2 + s^2}$, which removes the
$1/\sqrt{Q_x^2 - Q^2}$ singularity at the lower limit. A Bayesian Gaussian-process
alternative to the same inversion has been developed by Tung
\autocite{tung2026gpr}; the program implements the Abel method for its
determinism and its closed-form error propagation.

Equation~\eqref{eq:abel} evaluates $I$ at $Q_m \simeq \sigma_y$ and integrates
out to there, so it requires the scattering far beyond the \ac{usans} window.
This drives the one interactive decision in the workflow. When a single curve is
selected, the program asks---in the pane the user is already in, while the other
pane stays a live, filterable file list---whether a companion pinhole \ac{sans}
measurement of the same sample is available. If one is chosen, the two are put on
a common smearing basis, scale-matched, joined across the instrument gap, and
inverted over the full range. If the user declines, or when several curves are
desmeared at once, the high-$Q$ side is instead extrapolated as a power law out
to $\sigma_y$---the approach taken by the \ac{nist} \textsc{Igor}/\textsc{Irena}
package \autocite{kline2006}---and the result is truncated to the measured
\ac{usans} range, with the extrapolation recorded in the output file's header.
Huang \autocite{huang2026abel} show that without genuine high-$Q$ data the
reconstruction is reliable only at low $Q$; the truncation and the header note
make that limitation explicit rather than silent.

The desmeared curve is written as an ordinary three-column
$Q$, $I$, $\mathrm{d}I$ text file that the \key{p} key plots like any other. The
inversion is pure \texttt{numpy}, with no new dependency, and was validated by a
smear--desmear round trip against analytic power laws: over the core of the
\ac{usans} window the recovered log--log slope is within half a percent and the
median intensity error is below one percent, while the reported uncertainty
widens toward the top of the range where the high-$Q$ extrapolation loses
authority.
...
The suite comprises approximately 800 tests across 40 test modules, of which
all pass and two are skipped as environment-dependent (they require a real
NeXus file or the installed reduction engine). It runs in about 140~s. Five
tests are interface snapshot comparisons.
...
\begin{table}[ht]
  ...
  \begin{tabular}{p{0.55\textwidth} l l}
    ...
    \midrule
    Cold start to first paint & 300~ms & $\sim$120~ms median\tnote{a} \\
...
The \ac{usans} workflow of Section~\ref{sec:usans} has been exercised end to end
against a single proposal's data. Its grouping and scan-count reconciliation are
driven by conventions---consecutive same-title blocks, a trailing transmission
run, empty-cell titles naming a banjo---that hold for the experiments examined
but are not enforced by the data format. This is why the generated table is
presented as a draft for review rather than as an input to be reduced
unattended.
...
Table~\ref{tab:allcommands} lists every command registered in version 0.10.2.
...
\begin{longtable}{p{0.24\textwidth} p{0.20\textwidth} p{0.46\textwidth}}
  ...
  \texttt{usans.desmear} (\texttt{desmear}) & \texttt{paths?},
    \texttt{sans?}, \texttt{out\_dir?}, \texttt{sigma\_y?} & Desmear
