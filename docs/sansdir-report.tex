...
Figure~\ref{fig:iqxqy-tile} shows the tiled view on a real temperature series:
eight reduced $I(q_x,q_y)$ files from one sample measured from 20 to
55\,$^{\circ}$C. In the file pane the whole series is selected with one glob
tag---\key{+}, then \texttt{*C\_conf1\_Iqxqy.dat}---and \key{p} does the rest:
the files sort in natural order, tile four across, and share one logarithmic
color scale, so the evolution of the scattering ring with temperature reads
directly across the panels.
...
Figures~\ref{fig:metadata-workflow} and~\ref{fig:metadata-workflow-2} walk through a complete extract-and-plot
...
Figures~\ref{fig:usans-workflow} and~\ref{fig:usans-workflow-2} show the complete flow on a real proposal
