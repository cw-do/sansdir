...
use for \ac{sns} \ac{sans} workflows and is installed on the analysis cluster,
launched as \texttt{/SNS/EQSANS/shared/bin/sansdir}.
...
The primary deployment target is the \ac{ornl} analysis cluster, where a
released, non-editable install lives at
\texttt{/SNS/EQSANS/shared/script/sansdir-stable} together with a
self-contained Python virtual environment, and is exposed through the shared
tools directory as \texttt{/SNS/EQSANS/shared/bin/sansdir}. Every cluster user
has read access to those paths, so no per-user installation step is required.
The program is launched by running that command from any terminal on the
...
second argument is a starting path: the directory the left pane opens into, so a
